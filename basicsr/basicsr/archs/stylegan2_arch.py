...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, x, style):
        ...
        if self.demodulate:
            # torch.rsqrt针对输入input的每个元素的平方根的倒数来返回一个新的Tensor
            # pow(2)对各个元素求平方
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, resample_kernel=(1, 3, 3, 1), relu_type='prelu', norm_type='bn',
                 hg_depth=3, att_name='spar'):
...
class HourGlassBlock(nn.Module):
    ...
    def __init__(self, depth, c_in, c_out,
        ...
        if self.depth:
            ...
            self.out_block = nn.Sequential(
                ConvolutionalLayer(self.c_mid, self.c_out, norm_type='none', relu_type='none'),
                nn.Sigmoid()
            )
    ...
    def _generate_network(self, level):
        ...
        self.add_module('b1_' + str(level), ConvolutionalLayer(c1, c2, **self.kwargs))
        self.add_module('b2_' + str(level), ConvolutionalLayer(c1, c2, scale='down', **self.kwargs))
        if level > 1:
            self._generate_network(level - 1)
        else:
            self.add_module('b2_plus_' + str(level), ConvolutionalLayer(self.c_mid, self.c_mid, **self.kwargs))

        self.add_module('b3_' + str(level), ConvolutionalLayer(self.c_mid, self.c_mid, scale='up', **self.kwargs))
...
'''
    (1) 7层残差模块，通道数成指数级变小
    (2) 通过一个kernel = 3的卷积层，附有激活和正则化
    (3) 再经过两个全连接层得到最终结果，输出通道数为1
'''
