...
from .prior_estimation_arch import PriorEstimationNetwork
...
class GFPGANv1(nn.Module):
    ...
    def __init__(
        ...
        # out_size = 512, self.log_size = 9, first_out_size = 512
        ...
        # self.prior_Estimation_Network = PriorEstimationNetwork(128, 256, norm_layer)
    ...
    def forward(self, x, return_latents=False, return_rgb=True, randomize_noise=True, **kwargs):
        ...
        # encoder
        # feat = {Tensor:(1, 16, 512, 512)}
        feat = self.conv_body_first(x)
        for i in range(self.log_size - 2):
            feat = self.conv_body_down[i](feat)
            unet_skips.insert(0, feat)
        # unet_skips[0] = {Tensor:(1, 256, 4, 4)}
        # unet_skips[1] = {Tensor:(1, 256, 8, 8)}
        # unet_skips[2] = {Tensor:(1, 256, 16, 16)}
        # unet_skips[3] = {Tensor:(1, 256, 32, 32)}
        # unet_skips[4] = {Tensor:(1, 128, 64, 64)}
        # unet_skips[5] = {Tensor:(1, 64, 128, 128)}
        # unet_skips[6] = {Tensor:(1, 32, 256, 256)}
        ...
        for i in range(self.log_size - 2):
            ...
            # ResUpLayer
            # i = 0, feat = {Tensor:(1, 256, 4, 4)}
            # i = 3, feat = {Tensor:(1, 128, 64, 64)}
