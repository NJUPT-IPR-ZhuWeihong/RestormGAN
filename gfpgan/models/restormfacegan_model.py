...
from gfpgan.models.base_model2 import BaseModel2
...
class RESTORMFACEGANModel(BaseModel2):
    """The RESTORMGANModel model for Towards real-world blind face restoratin with generative facial prior"""

    def __init__(self, opt):
        super(RESTORMFACEGANModel, self).__init__(opt)
        self.use_facial_mask = None
        self.idx = 0  # it is used for saving data for check

        # define network
        # gfpgan/arches/gfpganv1_arch.py
    ...
    def init_training_settings(self):
        ...
        # ----------- define net_d ----------- #
        # basicsr/basicsr/archs/stylegan2_arch.py
    ...
    def setup_optimizers(self):
        ...
        betas = (0.9 ** net_g_reg_ratio, 0.999 ** net_g_reg_ratio)
        self.optimizer_g = self.get_optimizer(optim_type, optim_params_g, lr, weight_decay=weight_decay, betas=betas)
    ...
    def get_roi_regions(self, eye_out_size=80, mouth_out_size=120):
        ...
        # real images
        # RoIAlign 用于将任意尺寸感兴趣区域的特征图，都转换为具有固定尺寸H×W的小特征图。

        # 与RoI pooling一样，其基本原理是将h×w的特征划分为H×W网格，
        # 每个格子是大小近似h/H × w/W的子窗口，然后将每个子窗口中的值最大池化到相应的输出网格单元中。
    ...
    def get_mask(self):
        ...
        for b in range(self.loc_left_eyes.size(0)):  # loop for batch size
            array = self.lq[b]
            # x1, y1, x2, y2 = self.loc_left_eyes[b]
            # array[:, int(x1):int(x2), int(y1):int(y2)] = 1
            # x1, y1, x2, y2 = self.loc_right_eyes[b]
            # array[:, int(x1):int(x2), int(y1):int(y2)] = 1
            # x1, y1, x2, y2 = self.loc_mouths[b]
            # array[:, int(x1):int(x2), int(y1):int(y2)] = 1
            face_arrays.append(array)

        dim = 16
        for b in range(self.loc_left_eyes.size(0)):  # loop for batch size
            rgb_mask = torch.stack(face_arrays, dim=0)
            inter_mask1 = rgb_mask[b, 0, :, :].unsqueeze(0).repeat(dim * 2, 1, 1)
            mask1.append(inter_mask1)

            inter = F.interpolate(rgb_mask, scale_factor=0.5, mode='bilinear', align_corners=False)
            inter_mask2 = inter[b, 0, :, :].unsqueeze(0).repeat(dim * 2, 1, 1)
            mask2.append(inter_mask2)

            inter = F.interpolate(inter, scale_factor=0.5, mode='bilinear', align_corners=False)
            inter_mask3 = inter[b, 0, :, :].unsqueeze(0).repeat(dim * 4, 1, 1)
            mask3.append(inter_mask3)

        self.mask.append(torch.stack(mask3, dim=0))
        self.mask.append(torch.stack(mask2, dim=0))
        self.mask.append(torch.stack(mask1, dim=0))
    ...
    def resize_for_parsing(self, out, size=512):
        out_parsing = F.interpolate(out, (size, size), mode='bilinear', align_corners=False)
        return out_parsing
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.use_facial_mask = True
        if self.use_facial_mask:
            self.get_mask()
        ...
        if pyramid_loss_weight > 0:
            self.output, out_rgbs = self.net_g(self.lq, return_rgb=True, mask=self.mask)
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            # image pyramid loss
            # if pyramid_loss_weight > 0:
            #     for i in range(0, self.log_size - 5):
            #         l_pyramid = self.cri_l1(out_rgbs[i], pyramid_gt[i + 3]) * pyramid_loss_weight
            #         l_g_total += l_pyramid
            #         loss_dict[f'l_p_{2 ** (i + 3)}'] = l_pyramid
            if pyramid_loss_weight > 0:
                l_pyramid = self.cri_l1(out_rgbs[0], pyramid_gt[6]) * pyramid_loss_weight
                l_g_total += l_pyramid
                loss_dict[f'l_p_{2 ** 3}'] = l_pyramid
        ...
        # 其grad_fn=None且requires_grad=False，得到的这个tensor永远不需要计算其梯度，不具有梯度grad。
    ...
    def test(self, count):
        with torch.no_grad():
            if hasattr(self, 'net_g_ema'):
                self.net_g_ema.eval()
                self.output, _ = self.net_g_ema(x=self.lq, mask=self.face_arrays[count])
            else:
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            self.get_infer_mask(labelpath="D:/FSR_Code/RestormGAN/datasets/faces/validation/annotations/")
            self.test(count=idx)
    ...
    def get_infer_mask(self, labelpath):
        array = self.lq
        ...
        for file in filelist:
            ...
            with open(labelpath + file, 'r') as txt:
                ...
                face_arrays.append(array)
                self.face_arrays.append(face_arrays)
                txt.close()
