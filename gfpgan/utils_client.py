...
class GFPGANer2():
    ...
    def __init__(self, model_path, upscale=2, arch='clean', channel_multiplier=2, bg_upsampler=None, device=None, opt=None):
        ...
        if arch == 'clean':
            ...
            from gfpgan.archs.restormgan_arch import RESTORMGAN
            self.gfpgan = RESTORMGAN(num_blocks=[1, 2, 2, 3])
        elif arch == 'urestorm':
            from gfpgan.archs.Urestormgan_arch import URESTORMGAN
            self.gfpgan = URESTORMGAN()
        elif arch == 'restormface':
            from gfpgan.archs.restormgan_face_arch import RESTORMFACEGAN
            self.gfpgan = RESTORMFACEGAN()
        elif arch == 'restormhalf':
            from gfpgan.archs.restormgan_half_arch import RESTORMGANHALF
            self.gfpgan = RESTORMGANHALF()
        elif arch == 'GFPGAN_res':
            from gfpgan.archs.gfpgan_resblock_arch import GFPGANResBlock
            self.gfpgan = GFPGANResBlock(channel_multiplier=channel_multiplier,
                                         decoder_load_path='experiments/pretrained_models/StyleGAN2_512_Cmul1_FFHQ_B12G4_scratch_800k.pth')
        elif arch == 'GFPGAN_u':
            from gfpgan.archs.gfpgan_ublock_arch import GFPGANUBlock
            self.gfpgan = GFPGANUBlock()
        elif arch == 'GFPGAN_u_cff':
            from gfpgan.archs.gfpgan_ublock_arch import GFPGANUBlock
            self.gfpgan = GFPGANUBlock(channel_multiplier=channel_multiplier,
                                       decoder_load_path='experiments/pretrained_models/StyleGAN2_512_Cmul2_CFF_30k_chann=2.pth')
        elif arch == 'GFPGAN_u_latents':
            from gfpgan.archs.gfpgan_ublock_linear_arch import GFPGANUBlock2
            self.gfpgan = GFPGANUBlock2(channel_multiplier=channel_multiplier,
                                        decoder_load_path='experiments/pretrained_models/StyleGAN2_512_Cmul2_CFF_30k_chann=2.pth')
        elif arch == 'Uformer':
            from gfpgan.archs.Uformergan_arch import UformerGAN
            self.gfpgan = UformerGAN(input_is_latent=True,
                                     different_w=True,
                                     sft_half=True)
        elif arch == 'resblock2':
            from gfpgan.archs.gfpgan_resblock2_arch import GFPGANResBlock2
            self.gfpgan = GFPGANResBlock2(out_size=512,
                                          num_style_feat=512,
                                          channel_multiplier=channel_multiplier,
                                          decoder_load_path=None,
                                          fix_decoder=False,
                                          ...
                                          narrow=1,
                                          sft_half=True)
        elif arch == 'RestormGAN1223':
            from gfpgan.archs.restormgan_arch import RESTORMGAN
            self.gfpgan = RESTORMGAN(num_blocks=[1, 2, 2, 3])
        elif arch == 'RestormGAN1223_cbam':
            from gfpgan.archs.restormgan_cbam_arch import RESTORMGANCBAM
            self.gfpgan = RESTORMGANCBAM(num_blocks=[1, 2, 2, 3])
    ...
    @torch.no_grad()
    def enhance(self, img, img_path, has_aligned=False, only_center_face=False, paste_back=True, weight=0.5, count=None,
                with_mask=False):
