...
if __name__ == '__main__':
    ...
    parser.add_argument("--light_path", default='data_numpy/lighting.npy', type=str,
                        help="path to the lighting parameters")
    parser.add_argument("--attributes_path", default='data_numpy/attributes.npy', type=str,
                        help="path to the attribute parameters")
    parser.add_argument(
    ...
    my_dataset = MyDataset(latents=torch.Tensor(sg_latents).cuda(),
                           attributes=torch.tensor(sg_attributes).float().cuda())
    ...
    with tqdm(range(args.epochs)) as pbar:
        for epoch in pbar:
            for i, x in enumerate(train_loader):
                ...
                approx21, delta_log_p2 = prior(x[0].squeeze(1), x[1],
                                               torch.zeros(args.batch, x[0].shape[2], 1).to(x[0]))
